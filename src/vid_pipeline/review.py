    ...
    *,
    ...
    manifest = {
        ...
        "source_sha256": {
            ...
            "final_text": sha256_file(final_path),
        },
        "files": {
    ...
    (review_dir / "review.md").write_text(
        review_markdown(manifest, items),
        encoding="utf-8",
    )
    (review_dir / "review.html").write_text(
        review_html(manifest, items),
        encoding="utf-8",
    )
...
def apply_human_review(
    ...
    *,
    ...
    if unresolved:
        raise ReviewError(
            "Human review is incomplete; unresolved required items: "
            + ", ".join(unresolved)
        )
    ...
    for index, segment in enumerate(raw.get("segments") or []):
        ...
        if decision == "accept_suggestion" and segment_id in review_map:
            text = normalize_text(
                str(review_map[segment_id].get("proposed_text") or source)
            )
        elif decision == "edit":
    ...
    paragraphs = _paragraphs(
        (item["reviewed_text"] for item in reviewed),
        paragraph_words,
    )
    ...
    markdown_path.write_text(
        f"# متن بازبینی‌شده انسانی\n\n> بازبین: {reviewer_name}  \n"
        f"> تاریخ بازبینی: {utc_now()}  \n"
