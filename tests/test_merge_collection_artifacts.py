...
def _write_timing(root: Path, number: int, marker: str) -> None:
    path = root / "timings" / f"{number}.json"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def test_timing_sidecar_is_allowed_and_filled_when_base_is_already_complete(
    tmp_path: Path,
) -> None:
    staging = tmp_path / "staging"
    target = tmp_path / "target"
    _write_result(staging, 3, "checkpoint")
    _write_result(target, 3, "main")
    _write_timing(staging, 3, "checkpoint")
    ...
    assert result.returncode == 0, result.stderr
    assert (target / "timings/3.json").read_text(encoding="utf-8") == (
        '{"marker":"checkpoint"}\n'
    )
    assert "Merged 1 new collection files" in result.stdout
